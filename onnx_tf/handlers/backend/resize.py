...
@ps_description(
    ...
    "For opset 11, only the following attributes and inputs conbination " +
    "are supported in Tensorflow: 1. mode=nearest, " +
    "coordinate_transformation_mode=asymmetric, nearest_mode=floor, " +
    "can use scales(*) or sizes. 2. mode=nearest, " +
    "coordinate_transformation_mode=tf_half_pixel_for_nn, " +
    "nearest_mode=floor, can use scales(*) or sizes. 3. mode=linear, " +
    "coordinate_transformation_mode=half_pixel, can use scales(*) or sizes. " +
    "4. mode=cubic, coordinate_transformation_mode=half_pixel, " +
    "cubic_coeff_a=-0.5, exclude_outside=1, can use scales(*) or sizes. " +
    "5. mode=nearest, coordinate_transformation_mode=tf_crop_and_resize, " +
    "extrapolation_value=any_float_value, nearest_mode=round_prefer_ceil, " +
    "can use scales or sizes. 6. mode=linear, " +
...
class Resize(BackendHandler):
  ...
  def args_check(cls, node, **kwargs):
    ...
    if cls.SINCE_VERSION >= 11:
      ...
      if cubic_coeff_a == -0.75 and mode == "cubic":
        exception.OP_UNSUPPORTED_EXCEPT(
            "Resize mode=cubic and cubic_coeff_a=-0.75", "Tensorflow")
      if mode == "nearest":
        if (nearest_mode in [
